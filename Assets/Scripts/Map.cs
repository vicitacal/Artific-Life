            ...
            for (int y = 0; y < MapCreator.MapSixeY; y++)
            {
                switch (_viewMod)
                {
                    case ViewModes.IlluminationMode:
                        color = _illuminationGradient.Evaluate(IlluminationField.Values[x, y] / 50f);
                        break;
                    case ViewModes.OrganicMode:
                        color = _organicGradient.Evaluate(OrganicField.Values[x, y] / 500f);
                        break;
                    case ViewModes.ChargeMode:
                        color = _energyGradient.Evaluate(ChargeField.Values[x, y] / 500f);
                        break;
                }
